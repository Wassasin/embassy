...
use crate::pac::crc0::vals::{Fxor, Tcrc, Tot, Totr};
use embassy_hal_internal::Peri;
use nxp_pac::crc0::vals::Was;
...
impl<'d, M: Mode> Crc<'d, M> {
    ...
    fn regs() -> &'static crate::pac::crc0::Crc0 {
        &crate::pac::CRC0
    }
...
mod sealed {
    ...
    pub trait SealedWord: Copy {
        fn write(regs: &'static crate::pac::crc0::Crc0, word: Self);
        fn read(regs: &'static crate::pac::crc0::Crc0) -> Self;
    }
}
...
fn write_u8(regs: &'static crate::pac::crc0::Crc0, word: u8) {
    regs.data8().write(|w| *w = word);
}

fn read_u8(regs: &'static crate::pac::crc0::Crc0) -> u8 {
    regs.data8().read()
}

fn write_u16(regs: &'static crate::pac::crc0::Crc0, word: u16) {
    regs.data16().write(|w| *w = word);
}

fn read_u16(regs: &'static crate::pac::crc0::Crc0) -> u16 {
...
fn write_u32(regs: &'static crate::pac::crc0::Crc0, word: u32) {
    regs.data32().write(|w| *w = word);
}

fn read_u32(regs: &'static crate::pac::crc0::Crc0) -> u32 {
    regs.data32().read()
}
