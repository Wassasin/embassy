...
use nxp_pac::common::{RW, Reg};
use nxp_pac::gpio::vals::{Irqc, Isf};
use nxp_pac::port::vals::Ibe;
use paste::paste;

use crate::pac::interrupt;
use crate::pac::port::vals::{Dse, Inv, Mux, Pe, Ps, Sre};
...
fn irq_handler(port_index: usize, gpio_base: *const crate::pac::gpio::Gpio) {
...
#[interrupt]
fn GPIO0() {
    irq_handler(0, &crate::pac::GPIO0);
}

#[interrupt]
fn GPIO1() {
    irq_handler(1, &crate::pac::GPIO1);
}

#[interrupt]
fn GPIO2() {
    irq_handler(2, &crate::pac::GPIO2);
}

#[interrupt]
fn GPIO3() {
    irq_handler(3, &crate::pac::GPIO3);
}

#[interrupt]
fn GPIO4() {
    irq_handler(4, &crate::pac::GPIO4);
}
...
pub struct AnyPin {
    port: usize,
    pin: usize,
    gpio: &'static crate::pac::gpio::Gpio,
    port_reg: &'static crate::pac::port::Port,
    pcr_reg: Reg<crate::pac::port::regs::Pcr, RW>,
}
...
impl AnyPin {
    ...
    #[inline(always)]
    fn gpio(&self) -> &'static crate::pac::gpio::Gpio {
        self.gpio
    }
    ...
    #[inline(always)]
    fn port_reg(&self) -> &'static crate::pac::port::Port {
        self.port_reg
    }

    #[inline(always)]
    fn pcr_reg(&self) -> Reg<crate::pac::port::regs::Pcr, RW> {
        self.pcr_reg
    }
}
...
impl SealedPin for AnyPin {
    ...
    fn set_enable_input_buffer(&self) {
        self.pcr_reg().modify(|w| w.set_ibe(Ibe::IBE1));
    }
}
...
// Note: commented out pins do not have PORT entry.
...
impl<'d> Flex<'d> {
    ...
    pub fn set_as_input(&mut self) {
        ...
        gpio.pddr().modify(|w| w.0 = w.0 & !mask);
    }
    ...
    pub fn set_high(&mut self) {
        self.gpio().psor().write(|w| w.0 = self.mask());
    }

    /// Set output level to Low.
    #[inline]
    pub fn set_low(&mut self) {
        self.gpio().pcor().write(|w| w.0 = self.mask());
    }
    ...
    pub fn toggle(&mut self) {
        self.gpio().ptor().write(|w| w.0 = self.mask());
    }

    /// Get whether the pin input level is high.
    #[inline]
    pub fn is_high(&self) -> bool {
        (self.gpio().pdir().read().0 & self.mask()) != 0
    }
...
impl<'d> Flex<'d> {
    ...
    async fn wait_for_inner(&mut self, level: crate::pac::gpio::vals::Irqc) {
        ...
        self.pin.gpio().icr(self.pin.pin()).write(|w| w.set_isf(Isf::ISF1));

        // Pin interrupt configuration
        self.pin.gpio().icr(self.pin.pin()).modify(|w| w.set_irqc(level));
