...
bind_interrupts!(
    struct Irqs {
        LPI2C0 => target::InterruptHandler<LPI2C0>;
        LPI2C3 => controller::InterruptHandler<LPI2C3>;
    }
);
...
async fn main(spawner: Spawner) {
    ...
    let target = target::I2c::new_async(p.LPI2C0, p.P0_17, p.P0_16, Irqs, config).unwrap();

    spawner.spawn(target_task(target).unwrap());

    let config = controller::Config::default();
    let mut i2c = controller::I2c::new_async(p.LPI2C3, p.P3_21, p.P3_20, Irqs, config).unwrap();
